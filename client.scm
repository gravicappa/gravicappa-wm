...
(define (remove-client! c where s)
  (where s (remove c (where s))))

(define (move-client-to-top! c where s)
  (where s (cons c (remove c (where s)))))
...
(define (manage-client c)
  (let ((s (current-screen))
    ...
    (if (null? (client-tags c))
        (set-client-tags! c (list (current-tag))))
    (update-tag-hook)
    (set-client-floating! c (or (client-floating? c)
                                (client-fixed? c)
                                (client-fullscreen? c)))
    (x-raise-window dpy w)
    (add-client! c clients-list s)
    (if (not (memq c (clients-stack s)))
        (add-client! c clients-stack s))
  ...
  (let ((w (client-window c))
    ...
    (dynamic-wind
      ...
      (lambda ()
        ...
        (remove-client! c clients-list s)
        (remove-client! c clients-stack s)
        (x-ungrab-button dpy x#+any-button+ x#+any-modifier+ w)
